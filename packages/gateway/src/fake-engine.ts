...
// Listens on http://localhost:8000/evaluate, takes { action, context } and answers in camelCase, like Person 2's engine.
...
createServer((req, res) => {
  ...
  req.on('end', () => {
    ...
    const { action = {}, context = {} } = JSON.parse(raw || '{}');
    ...
    console.error(`[fake-engine] ${action.agentId}/${context.sessionId} ${action.toolName} -> ${decision}`);

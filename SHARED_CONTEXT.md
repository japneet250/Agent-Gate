...
**Branches:** code lives on `aaryan` (P1) / `person2/engine` (P2) / `person3` (P3).
**This file now lives on `person3`, not `main`** — read it with
`git show person3:SHARED_CONTEXT.md` or pull the branch.
**`main` is deliberately empty** (upstream LICENSE + README only) until the team
integrates everything and merges it as one reviewed step; nothing goes there
piecemeal. Only touch your own section; the Decisions Log is append-only.
...
- [20:40] (P3) **Branch model changed at the user's direction: `main` goes back to the upstream initial commit and stays empty until the team integrates everything and merges it once.** `SHARED_CONTEXT.md` now lives on **`person3`** (`git show person3:SHARED_CONTEXT.md`); nothing is pushed to `main` piecemeal any more. The old main-only rule also caused real damage: `report.json` was tracked on `main` and ignored on `person3`, so every doc round-trip silently destroyed the `--model=engine` baseline — twice, at a full 100-scenario re-run each time.
